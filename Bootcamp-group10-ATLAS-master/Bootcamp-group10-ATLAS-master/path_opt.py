...
def logic_topo_gen(graph, conges_edge_start,conges_edge_end):
    '''
	This fuction is use to construct the logic map 
	due to the time, we omit the complex operation
	neet to implement in the next version
	'''
...
def cal_optimal_path(graph,keylist):
	'''This fuction used to recaculate the path during the congestion
	This is the simple version that donot concern of flow divided.
	It need to extend in the next version.
	'''
    opt_path = [keylist[0]]
